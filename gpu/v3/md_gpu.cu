...
__global__
void useTheForce(atom * atom_list_old, atom * atom_list_new, double ts, int n) 
{
	...
	if (i < n)
	{
		...
		prev_ax = atom_list_new[i].ax;
		prev_ay = atom_list_new[i].ay;
		prev_az = atom_list_new[i].az;

		// a = F/m
		atom_list_new[i].ax = atom_list_new[i].fx / atom_list_new[i].mass;
		atom_list_new[i].ay = atom_list_new[i].fy / atom_list_new[i].mass;
		atom_list_new[i].az = atom_list_new[i].fz / atom_list_new[i].mass;

		// new velocity
		atom_list_new[i].vx = atom_list_new[i].vx + 0.5*(atom_list_new[i].ax + prev_ax)*ts;
		atom_list_new[i].vy = atom_list_new[i].vy + 0.5*(atom_list_new[i].ay + prev_ay)*ts;
		atom_list_new[i].vz = atom_list_new[i].vz + 0.5*(atom_list_new[i].az + prev_az)*ts;
	...
	//cudaMemcpy(d_atom_list_new, atom_list, atoms_size, cudaMemcpyHostToDevice);
		...
		cudaEventRecord( start, 0 );
			...
			for (float t=0.0; t <= tf; t+=ts) {
				...
				useTheForce<<< dimGrid, dimBlock >>>( d_atom_list_old, d_atom_list_new, ts, n );
				...
				if (c%1 == 0)
				{
					...
					cudaMemcpy(atom_list, d_atom_list_new, atoms_size, cudaMemcpyDeviceToHost);
