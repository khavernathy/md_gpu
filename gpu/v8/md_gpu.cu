...
void calculateForce(atom * atom_list, double ts, int n,double dimx,double dimy,double dimz)
{
	...
	if (i < n) 
	{
		...
		for (int j=i+1; j<n; j++) 
		{
			...
			//printf("atomi atomj %f %f\n",atom_list[i].pz,atom_list[j].pz);
			
			
			//calculate distances of closest image
			...
			//if(i==37) 
			//printf("dx1 %f ",dx*1e10);
			
			dx-=sign* ( (int)abs(dx*1e10/dimx) )*dimx*1e-10;
			//if(i==37)
			//printf("dx2 %f ",dx*1e10);
			dx-=sign* ( (int)abs(dx*1e10/(dimx/2)) )*dimx*1e-10;
			...
			dy-=sign*( (int)abs(dy*1e10/dimy) )*dimy*1e-10;
                        dy-=sign*( (int)abs(dy*1e10/(dimy/2)) )*dimy*1e-10;
			...
			dz-=sign*( (int)abs(dz*1e10/dimz) )*dimz*1e-10;
                        dz-=sign*( (int)abs(dz*1e10/(dimz/2)) )*dimz*1e-10;
...
void useTheForce(atom * atom_list, double ts, int n, double dimx, double dimy, double dimz) 
{
	...
	if (i < n)
	{
		...
		printf("%d\n", (int) ( (atom_list[i].px*1e10+ dimx/2)/dimx )  ) ;
		atom_list[i].px -= ( (int) ( (atom_list[i].px*1e10)+dimx/2)/dimx )*dimx*1e-10;
		
		atom_list[i].py -= ( (int) (atom_list[i].py*1e10+dimy/2)/dimy )*dimy*1e-10;
		atom_list[i].pz -= ( (int) (atom_list[i].pz*1e10+dimz/2)/dimz )*dimz*1e-10;
