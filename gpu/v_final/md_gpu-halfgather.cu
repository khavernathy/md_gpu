...
void calculateForce(atom * atom_list, double ts, int n,double dimx,double dimy,double dimz)
{
	...
	//loop through number of tiles = number of atoms/blocksize
	...
	do{
		...
		if(i2<=i || i2>=n){//condition from i2<=i to i2==i changed since all pair interactions are calculated twice 
			j=(j+1)%blockDim.x;
			continue;
			}
		...
		*/
		...
		atomicAdd(&(atom_list[i].fx),afx);
		atomicAdd(&(atom_list[i].fy),afy);
		atomicAdd(&(atom_list[i].fz),afz);
...
int main(int argc, char **argv)
{
	...
	System system; 
	double ts = 0.4e-15; // time step 1e-15 = 1 femptosecond.
